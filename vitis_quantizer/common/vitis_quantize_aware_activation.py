...
class QuantizeAwareActivation(object):
    ...
    def __init__(self, activation, quantizer, mode, step, quantize_wrapper):
        ...
        if self._should_pre_quantize():
            self._pre_activation_vars = quantizer.build(
                None, "pre_activation", quantize_wrapper
            )

        if self._should_post_quantize():
            self._post_activation_vars = quantizer.build(
                None, "post_activation", quantize_wrapper
            )
    ...
    def _is_supported_activation(self, activation):
        ...
        return (
            activation_name in self._PRE_QUANT_ACTIVATIONS
    ...
    def __call__(self, inputs, *args, **kwargs):
        ...
        if self._should_pre_quantize():
            x = tf_utils.smart_cond(
                ...
                make_quantizer_fn(
                    self.quantizer, x, False, self.mode, self._pre_activation_vars
                ),
            )
        ...
        if self._should_post_quantize():
            x = tf_utils.smart_cond(
                ...
                make_quantizer_fn(
                    self.quantizer, x, False, self.mode, self._post_activation_vars
                ),
            )
